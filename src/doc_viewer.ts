 ...
 * shown.
 */
async function loadDocSet(id: DocSetId): Promise<DocSet> {
  ...
  if (matcher.test(readme)) {
    ...
    config = <DocSet>JSON.parse(data);
  }
  if (config) {
    for (const key of Object.keys(config)) {
      const prop = <keyof DocSet>key;
      docSet[prop] = config[prop] as any;
    }
  }

  const pageNames = (docSet.pages = ['README.md'].concat(docSet.pages || []));

  docSet.ready = Promise.all(
    pageNames
      .filter(name => name !== 'README.md')
      .map(async name => {
        const response = await fetch(docBase + name);
        const text = await response.text();
        return renderPage(text, name, id);
      })
  ).then(() => {
...
function updateNavBarLinks(id: DocSetId) {
  const docSet = getDocSet(id);
  const navbar = <HTMLElement>document.querySelector('.docs-nav .navbar-start');

  navbar.classList[docSet.api ? 'add' : 'remove']('has-api');
  navbar.classList[docSet.pages ? 'add' : 'remove']('has-docs');

  const docTypes = <DocType[]>(
    Object.keys(DocType).filter(type => !Number(type))
  );
  for (const type of docTypes) {
    const link = <HTMLLinkElement>(
      navbar.querySelector(`.navbar-item[data-doc-type="${type}"]`)!
    );
...
function updateDocsetSelector() {
  ...
  const option = <HTMLOptionElement>(
    selector.querySelector(`option[value="${getCurrentPageId().project}"]`)
  );
  if (option) {
...
function highlightActiveSection() {
  ...
  let link = <HTMLElement>(
    menu.querySelector(`li > a[href="${currentSection}"]`)!
  );
  if (!link) {
    try {
  ...
  try {
    ...
    loadDocSet(docSetId)
      .then(docSet => {
        ...
        const ready = type === DocType.api ? docSet.apiReady! : docSet.ready!;
        ready
          .then(() => {
            const pageId = getCurrentPageId();
          ...
          .catch(error => {
            ...
            updateHash(
              createHash(getDefaultPageId(docSetId, type || DocType.docs)),
              HashEvent.rename
            );
            processHash();
          })
...
function updateGitHubButtons(docs: DocSetId) {
  const links = <NodeListOf<HTMLAnchorElement>>(
    document.querySelectorAll('.github-button')
  );
